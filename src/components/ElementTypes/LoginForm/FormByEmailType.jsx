...
const FormByEmailType = ({sign_in_email_opt}) => { 
    var showPassInput = sign_in_email_opt == 'guest_sign_email_pass_otp_both_req' || sign_in_email_opt == 'guest_sign_email_pass_only'
    var showOtpInput = sign_in_email_opt == 'guest_sign_email_pass_otp_both_req' || sign_in_email_opt == 'guest_sign_email_otp_only'
    ...
    const onFinish = async (values) => {
      ...
      ? { ...data, 
          sign_in_type: sign_in_email_opt === "guest_sign_email_pass_only" 
          ? "email_password" 
          : "email_otp" 
        }
      : { ...values, ...data, sign_in_type: "email_otp" };


      if(!otpInputShow &&  sign_in_email_opt == 'guest_sign_email_pass_only'){
      ...
      }else if(!otpInputShow &&  sign_in_email_opt != 'guest_sign_email_pass_only'){
        const res = await postLogin({ sendData: sendData });
        if(res?.res?.ok){
  ...
  return (
    ...
    <div className="flex gap-3 flex-col mb-3">
      {!otpInputShow ?
      <>
      <Form.Item style={{margin:0}} label={t('email')} name={'email'} rules={[{required: true} , {type: 'email'}]}>
...
export default FormByEmailType